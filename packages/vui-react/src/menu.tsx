...
import { MENU_ROOT, MENU_SECTION } from "./class-variants";
...
export function Menu({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div data-slot="menu" role="menu" className={cn(MENU_ROOT, className)} {...props}>
      {children}
    </div>
  );
}
...
export const MenuItem = React.forwardRef<HTMLButtonElement, MenuItemProps>(function MenuItem(
  { as: Comp = "button", className, children, ...props },
  ref,
) {
  return (
    <Comp
...
export function MenuLabel({
  ...
  return <p className={cn(MENU_SECTION, className)}>{children}</p>;
}
