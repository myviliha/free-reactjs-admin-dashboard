...
export type FormActionsConfig<T, Icon = unknown> =
  | FormAction<T, Icon>[]
  | ((defaults: FormAction<T, Icon>[]) => FormAction<T, Icon>[]);
...
export type IoActionsConfig<T, Icon = unknown> =
  | IoAction<T, Icon>[]
  | ((defaults: IoAction<T, Icon>[]) => IoAction<T, Icon>[]);
...
export function defineConfig<Icon = unknown>(config: VuiConfig<Icon>): VuiConfig<Icon> {
  return config;
}

/** Merge configs left to right; a later object overrides only the keys it sets. */
export function mergeConfig<Icon = unknown>(
  ...configs: (VuiConfig<Icon> | undefined)[]
): VuiConfig<Icon> {
  const out: VuiConfig<Icon> = {};
  for (const config of configs) {
    if (!config) continue;
    for (const [group, values] of Object.entries(config)) {
      if (values == null) continue;
      const key = group as keyof VuiConfig;
      out[key] = { ...(out[key] ?? {}), ...values } as VuiConfig<Icon>[keyof VuiConfig];
    }
  }
  return out;
}
...
export function filterUserPreferences<Icon = unknown>(
  preferences: VuiPreferences<Icon>,
  ...
  for (const [group, values] of Object.entries(preferences)) {
    ...
    if (Object.keys(kept).length) out[section] = kept as VuiConfig<Icon>[keyof VuiConfig];
  }
  return out;
}
