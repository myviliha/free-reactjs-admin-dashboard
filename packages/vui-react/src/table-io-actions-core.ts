...
import type { IoAction, IoActionsConfig, IoContext } from "./config-core";
import {
  downloadFile,
  type IoColumn,
  parseCSV,
...
export type IoIcons<Icon> = { csv?: Icon; excel?: Icon; json?: Icon; pdf?: Icon };

const fileBase = (title: string) => title.toLowerCase().replace(/\s+/g, "-") || "export";

/** Export: CSV, Excel, JSON, and PDF via the print dialog. */
export function defaultExportActions<T, Icon = unknown>(
  icons: IoIcons<Icon> = {},
): IoAction<T, Icon>[] {
 ...
 */
export function defaultImportActions<T extends { id: string | number }, Icon = unknown>(
  ...
  const read = async (ctx: IoContext<T>) => {
    ...
    const rows = records.map((record) => {
      const row = { ...makeEmptyRow(), id: nextId() } as Record<string, unknown>;
...
export function resolveIoActions<T, Icon = unknown>(
  defaults: IoAction<T, Icon>[],
  config: IoActionsConfig<T, Icon> | undefined,
): IoAction<T, Icon>[] {
