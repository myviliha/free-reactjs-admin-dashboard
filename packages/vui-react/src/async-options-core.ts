...
export const asValues = (v: string | string[] | undefined): string[] =>
 ...
 */
const inFlight = new WeakMap<object, Map<string, Promise<AsyncOption[]>>>();

export function share(
  fn: object,
  key: string,
  run: () => Promise<AsyncOption[]>,
): Promise<AsyncOption[]> {
  let byKey = inFlight.get(fn);
  if (!byKey) inFlight.set(fn, (byKey = new Map()));
...
const queues = new WeakMap<object, { ids: Set<string>; run: Promise<AsyncOption[]> | null }>();
...
export function batch(
  ...
  if (!q) queues.set(resolveOptions, (q = { ids: new Set(), run: null }));
  for (const id of ids) q.ids.add(id);
  const queue = q;
  if (!queue.run) {
