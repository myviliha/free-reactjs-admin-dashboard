...
export const WIZARD_PLANS: readonly { value: string; label: string }[] = [
 ...
 */
export function canLeaveRegistrationStep(
  step: number,
  values: { name: string; slug: string; email: string; password: string },
): boolean {
  if (step === 0) return values.name.trim() !== "" && values.slug.trim() !== "";
  if (step === 1) return values.email.trim() !== "" && values.password !== "";
  return true;
}
