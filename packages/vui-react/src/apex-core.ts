import { DASHBOARD_TARGET, MONTHLY_SALES, STATISTICS } from "./demo-tables-core";
...
export function apexBase(): ApexBaseOptions {
  return {
    chart: {
      fontFamily: "inherit",
      toolbar: { show: false },
...
export function barChartOptions(): {
  ...
  const bars = {
    ...
    tooltip: { ...base.tooltip, y: { formatter: (value: number) => `${value}` } },
  };
  const grouped = {
    ...bars,
...
export function monthlySalesOptions(months: readonly string[]): Record<string, unknown> {
  const base = apexBase();
  return {
    ...base,
    chart: { ...base.chart, type: "bar" },
...
export const LINE_SERIES: ApexSeries = [
...
export function statisticsOptions(categories: readonly string[]): Record<string, unknown> {
  const base = apexBase();
  return {
    ...base,
    chart: { ...base.chart, type: "area" },
...
export function monthlyTargetOptions(): Record<string, unknown> {
  const base = apexBase();
  return {
    ...base,
...
export const formatDayFirst = (date: Date) =>
  [
    String(date.getDate()).padStart(2, "0"),
    String(date.getMonth() + 1).padStart(2, "0"),
    date.getFullYear(),
 ...
 */
export const CALENDAR_LEVEL_RING: Record<(typeof CALENDAR_LEVELS)[number], string> = {
