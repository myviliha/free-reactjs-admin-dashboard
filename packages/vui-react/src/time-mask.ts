...
export function timeParts(value: string): { hour: string; minute: string; meridiem: string } {
  const match = /^(\d{2}):(\d{2})$/.exec(value);
  if (!match) return { hour: "", minute: "", meridiem: "" };
  const h24 = Number(match[1]);
  const meridiem = h24 >= 12 ? "PM" : "AM";
  const h12 = h24 % 12 === 0 ? 12 : h24 % 12;
  return { hour: `${h12}`.padStart(2, "0"), minute: match[2] as string, meridiem };
}

/** The three parts back to `HH:mm`. The one place 12-hour becomes 24. */
export function partsToValue(hour: string, minute: string, meridiem: string): string {
 ...
 */
export function maskTime(raw: string): MaskedTime {
  ...
  const explicit =
    groups.length === 2 && (groups[0] as string).length <= 2 && (groups[1] as string).length === 2;
  /*
  ...
  const digits = explicit
    ? `${(groups[0] as string).padStart(2, "0")}${groups[1]}`
    : threeIsHourMinute
      ? plain.padStart(4, "0")
      : plain;
  if (digits.length === 0) return { text: "", value: "", invalid: false };

  const hourPart = digits.slice(0, 2);
  const minutePart = digits.slice(2, 4);
  let text = hourPart;
  if (digits.length > 2) text += `:${minutePart}`;
  if (digits.length < 4) return { text, value: "", invalid: false };

  const h = Number(hourPart);
  const m = Number(minutePart);
  if (m > 59 || h > 23) return { text, value: "", invalid: true };
