 ...
 */
export function resolveFormActions<T, Icon = unknown>(
  defaults: FormAction<T, Icon>[],
  config: FormActionsConfig<T, Icon> | undefined,
): FormAction<T, Icon>[] {
...
export const actionRequiresValid = <T, Icon = unknown>(action: FormAction<T, Icon>): boolean =>
 ...
 */
export function defaultFormActions<T, Icon = unknown>({
  readOnly,
  canEdit,
  icons = {},
}: {
  readOnly: boolean;
  canEdit: boolean;
  icons?: { close?: Icon; edit?: Icon; cancel?: Icon; save?: Icon };
}): FormAction<T, Icon>[] {
  ...
  // and close.
  if (readOnly) {
    const actions: FormAction<T, Icon>[] = [
      {
        id: "close",
        label: "Close",
        icon: icons.close,
