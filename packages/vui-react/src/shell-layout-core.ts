 ...
 */
export const SHELL_LAYOUTS: readonly ShellLayout[] = [
  {
    id: "one",
    label: "Application",
...
export const layoutById = (id: string | null | undefined): ShellLayout =>
  SHELL_LAYOUTS.find((layout) => layout.id === id) ??
  // Never undefined: an unknown id in a URL or in storage is a typo, not a reason to render no shell.
  (SHELL_LAYOUTS.find((layout) => layout.id === DEFAULT_LAYOUT) as ShellLayout);

/** `true` when the layout is the icon-only rail, which has no room for labels or submenus. */
export const isRail = (layout: ShellLayout) => layout.brand === "icon";
