...
export type EmailCheck = { ok: true; domain: string } | { ok: false; reason: "format" | "public" };
...
export const PLANS: OnboardingPlan[] = [
  {
    id: "free",
    name: "Free",
    price: "$0",
...
export function canLeaveOnboardingStep(
  step: number,
  values: { firstName: string; lastName: string; companyName: string },
): boolean {
  if (step === 0) return values.firstName.trim() !== "" && values.lastName.trim() !== "";
  if (step === 1) return values.companyName.trim() !== "";
  return true;
}

/** The API token and the team invites are optional, so those two steps offer Skip. */
export const isSkippableOnboardingStep = (step: number) => step === 2 || step === 4;
 ...
 */
export function demoApiToken(): string {
  let body = "";
  while (body.length < DEMO_TOKEN_LENGTH) body += Math.random().toString(36).slice(2);
