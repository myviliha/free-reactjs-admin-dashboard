...
import type { FieldRules, FilterValues, RowId, SortState } from "./record-field-core";
...
export const RV_PAGE_SIZES = [10, 25, 50, 100] as const;
 ...
 */
export function computeColumnAligns<T extends { id: RowId }>(
  fields: readonly FieldRules<T>[],
  data: readonly T[],
...
export function fieldDefaultWidth<T>(field: FieldRules<T>): number {
...
): (FieldRules<T> | IdentitySlot)[] {
  const cols: (FieldRules<T> | IdentitySlot)[] = [...visibleFields];
 ...
 */
export function totalColumnWidth<T>(args: {
  visibleFields: readonly FieldRules<T>[];
  ...
  return (
    (args.showSelection ? RV_CHECKBOX_W : 0) +
    RV_ACTIONS_W +
    name +
...
export const resizedWidth = (from: number | undefined, delta: number): number =>
  Math.max(RV_MIN_COL_W, (from ?? RV_RESIZE_BASE_W) + delta);
 ...
 */
export const canSortField = <T>(f: FieldRules<T>): boolean => f.sortable ?? !f.hideInTable;
 ...
 */
export function resolveNameSortKey<T>(
  fields: readonly FieldRules<T>[],
  nameSortKey?: string,
): string | undefined {
...
export function clipCell(value: string, max: number): { text: string; full?: string } {
 ...
 */
export function pageSizeOptions(maxPageSize: number): number[] {
...
export const clampPageSize = (defaultPageSize: number, maxPageSize: number): number =>
...
export function clientFilter<T extends { id: RowId }>(
  rows: readonly T[],
  keyword: string,
  fields: readonly FieldRules<T>[],
  ...
  return rows.filter((row) => {
    if (getTitle(row).toLowerCase().includes(q)) return true;
 ...
 */
export function clientSort<T>(rows: readonly T[], sort: SortState | null): T[] {
  if (!sort) return [...rows];
  const { key, dir } = sort;
...
export const nextSort = (current: SortState | null, key: string): SortState =>
 ...
 */
export function pruneFilterValues<T>(
  fields: readonly FieldRules<T>[],
  values: FilterValues<T>,
): FilterValues<T> | null {
  let changed = false;
  const next: FilterValues<T> = { ...values };
  for (const f of fields) {
    ...
    const v = values[f.key as keyof FilterValues<T>];
    if (typeof v === "string" && v && !valid.has(v)) {
      next[f.key as keyof FilterValues<T>] = "" as never;
      changed = true;
    } else if (Array.isArray(v)) {
      const kept = v.filter((x) => valid.has(x));
      if (kept.length !== v.length) {
        next[f.key as keyof FilterValues<T>] = kept as never;
        changed = true;
      }
    }
  }
 ...
 */
export function reorderRows<T extends { id: RowId }>(
  rows: readonly T[],
  sourceId: RowId,
  targetId: RowId,
): T[] {
