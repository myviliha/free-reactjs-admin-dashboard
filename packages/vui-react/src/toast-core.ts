...
export type ToastItem<D = unknown> = ToastOptions<D> & { id: number };
 ...
 * never re-render.
 */
export function getToasts<D = unknown>(): readonly ToastItem<D>[] {
  return items as ToastItem<D>[];
}
...
export const toast: ToastFn = Object.assign(
  (opts: ToastOptions | string) => addToast(typeof opts === "string" ? { title: opts } : opts),
  {
    success: (title: string, opts?: Extra<unknown>) =>
      addToast({ ...opts, title, variant: "success" }),
    error: (title: string, opts?: Extra<unknown>) => addToast({ ...opts, title, variant: "error" }),
    warning: (title: string, opts?: Extra<unknown>) =>
