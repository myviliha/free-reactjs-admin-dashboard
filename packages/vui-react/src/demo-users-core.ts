...
// Re-exported so `@viliha/vui-core/demo-users` keeps its whole surface: the shape and the value sets
// moved to a stateless module in review, and no consumer should have to know that.
export type { User, UserStatus } from "./demo-user-options-core";
export { ROLES, STATUSES, TEAMS } from "./demo-user-options-core";
...
export function listUsers(
  ...
  return new Promise((resolve, reject) => {
    const t = setTimeout(() => {
      ...
      if (search) {
        out = out.filter((u) =>
          [u.name, u.email, u.role, u.team].some((v) => v.toLowerCase().includes(search)),
        );
      }
