...
import {
  ...
  CONFIRM_DIALOG_DESTRUCTIVE_ICON,
  CONFIRM_DIALOG_PANEL,
} from "./class-variants";
...
export function ConfirmDialog({
  ...
  return (
    <Dialog open={open} onClose={onCancel} label={title} className={CONFIRM_DIALOG_PANEL}>
      <DialogHeader>
        <DialogTitle>{title}</DialogTitle>
      </DialogHeader>
      {description && <DialogBody className={CONFIRM_DIALOG_DESCRIPTION}>{description}</DialogBody>}
      <DialogFooter>
        <Button onClick={onCancel}>{cancelLabel}</Button>
        <Button
          variant={destructive ? "destructive" : "primary"}
          onClick={onConfirm}
          className={cn(destructive && CONFIRM_DIALOG_DESTRUCTIVE_ICON)}
        >
          {confirmLabel}
        </Button>
      </DialogFooter>
