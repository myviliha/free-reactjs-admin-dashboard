...
export type NavSectionData = { title?: string; items: NavEntryData[] };

export function isNavGroup(entry: NavEntryData): entry is NavGroupData {
  return (entry as NavGroupData).children !== undefined;
}

export const NAV_TREE: NavSectionData[] = [
 ...
 */
export const DEMO_ICON_RADIX: Record<string, string> = {
 ...
 */
export function sectionIndex(tree: NavSectionData[] = NAV_TREE): Record<string, string> {
...
export const PARITY_ROUTES: string[] = [
  "/",
  "/auth",
...
export function navLinks(tree: NavSectionData[] = NAV_TREE): NavLinkData[] {
...
export function navIconKeys(tree: NavSectionData[] = NAV_TREE): string[] {
  return [
    ...new Set(
 ...
 */
export const FREE_NAV: readonly FreeNavGroup[] = [
  {
...
export const FREE_NAV_HREFS: readonly string[] = FREE_NAV.flatMap((group) =>
  group.entries.flatMap((entry) =>
    entry.href ? [entry.href] : (entry.children ?? []).map((child) => child.href),
  ),
);
 ...
 */
export const FREE_ROUTES: string[] = [...FREE_NAV_HREFS].sort();
 ...
 */
export const initialsOf = (name: string, words?: number) =>
  name
    .split(" ")
