...
export type CopySegment = string | { text: string; href: string } | { code: string };
...
export const PRICING_LEAD =
  "VUI is free and MIT licensed, the data table included. Pro is the record workflow and the premium blocks, for teams whose data lives on a server and who want someone to ask.";
...
export function teamTier(contactEmail: string): PricingTier {
  return {
    amount: "Team",
    cadence: "talk to us",
...
export const PRICING_NOT_ON_SALE: readonly CopySegment[] = [
...
export const PRICING_SPONSOR: readonly CopySegment[] = [
...
export const isLink = (s: CopySegment): s is { text: string; href: string } =>
  typeof s === "object" && "href" in s;
export const isCode = (s: CopySegment): s is { code: string } =>
  typeof s === "object" && "code" in s;
