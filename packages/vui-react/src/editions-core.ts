...
export const FAMILY_AVAILABILITY: readonly FamilyAvailability[] = [
  {
    name: "accordion",
    react: "ships",
    vue: "ships",
  ...
  },
  {
    ...
    angularNote:
      "Presence is a wrapper the call site writes, `[vuiAvatarStatus]`, because an Angular directive " +
  ...
  },
  {
    ...
    angularNote:
      "Recharts is React-only. `tanstack-chart` is the cross-framework answer, and Angular ships its card.",
  },
  { name: "checkbox", react: "ships", vue: "ships", angular: "ships", tier: "free", html: "ships" },
  {
    ...
    note: "html: not a rendered family: it is provider or plumbing, and there is nothing to show",
    angularNote:
      "React's `Code` renders a bare `<code>` with no class of its own, so there is nothing for an edition to port.",
  },
  {
    name: "collapsible",
    react: "ships",
   ...
   */
  {
    ...
    angularNote:
      "Not ported yet. The joining is pure CSS, so the port is markup rather than logic.",
  },
  {
    name: "carousel",
    react: "ships",
    ...
    angularNote:
      "Not ported yet. It needs state for the current slide, so it is a real port rather than markup.",
  },
  {
    name: "list",
    react: "ships",
   ...
   */
  {
    ...
    angularNote:
      "Not ported yet. The Angular data table draws its own pager, so a paged table works; what is missing is the standalone family.",
  },
  ...
  { name: "steps", react: "ships", vue: "ships", angular: "ships", tier: "free", html: "ships" },
  {
    ...
    angularNote:
      "Angular cannot clone projected content, so it finds the control in its own host after render " +
      "rather than taking a second directive at the call site. Same usage as React and Vue.",
    note:
      "The HTML edition has no port: the border and the message are static markup, but the tooltip " +
  ...
  },
  {
    ...
    note:
      "The HTML edition has no port: the poster is static markup and the click that swaps it for the " +
...
export const familiesFor = (edition: Edition): readonly FamilyAvailability[] =>
  FAMILY_AVAILABILITY.filter((f) => f[edition] !== "absent");

/** Everything it does not, each with the reason. A gallery shows these rather than omitting them. */
export const missingFor = (edition: Edition): readonly FamilyAvailability[] =>
  FAMILY_AVAILABILITY.filter((f) => f[edition] === "absent");
 ...
 */
export const renderableFor = (edition: Edition): readonly FamilyAvailability[] =>
  FAMILY_AVAILABILITY.filter((f) => f[edition] !== "absent" || f.note === undefined);
