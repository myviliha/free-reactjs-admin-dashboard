import { FREE_RAIL_WIDTH } from "./class-variants";
...
export const submenuId = (label: string): string =>
  `submenu-${label.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;

/** The current route, compared exactly. A prefix match would light two items at once. */
export const isCurrent = (pathname: string, href: string): boolean => pathname === href;

/** A group is active when it is the current route or holds it. */
export const holdsCurrent = (entry: FreeNavEntry, pathname: string): boolean =>
 ...
 */
export function initialOpen(nav: readonly FreeNavGroup[], pathname: string): readonly string[] {
...
export function toggleGroup(open: readonly string[], label: string): readonly string[] {
  return open.includes(label) ? open.filter((l) => l !== label) : [...open, label];
}

/** Whether a group's submenu is open. */
export const isExpanded = (open: readonly string[], label: string): boolean => open.includes(label);
 ...
 */
export const isRailShowing = (layout: ShellLayout, collapsed: boolean): boolean =>
  isRail(layout) || collapsed;

/** The sidebar's width in pixels for the current layout and state. */
export function sidebarWidth(layout: ShellLayout, collapsed: boolean): number {
  if (isRail(layout)) return layout.width;
