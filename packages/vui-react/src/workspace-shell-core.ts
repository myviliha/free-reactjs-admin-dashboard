...
export const COLLAPSED_GROUP_MODES: readonly CollapsedGroupMode[] = [
  "inline",
  "flyout-click",
  "flyout-hover",
];
...
export const DEFAULT_COLLAPSED_GROUP_MODE: CollapsedGroupMode = "flyout-hover";
 ...
 */
export function collapsedGroupMode(raw: string | undefined): CollapsedGroupMode {
...
export const usesFlyout = (collapsed: boolean, mode: CollapsedGroupMode): boolean =>
 ...
 */
export const isGroup = (entry: ShellNavEntry): boolean =>
  Array.isArray(entry.children) && entry.children.length > 0;

/** A group is current when any of its children is. */
export const holdsCurrent = (entry: ShellNavEntry, pathname: string): boolean =>
  (entry.children ?? []).some(
    (child) => child.href !== undefined && isCurrent(pathname, child.href),
  );
 ...
 * per row.
 */
export function initialOpen(nav: readonly ShellNavSection[], pathname: string): Set<string> {
  const open = new Set<string>();
  for (const section of nav) {
    for (const entry of section.items) {
      if (holdsCurrent(entry, pathname)) open.add(entry.label);
    }
  }
  return open;
}
...
export function toggleGroup(
  ...
  if (flyout) return open.has(label) ? new Set() : new Set([label]);
...
export const isExpanded = (open: ReadonlySet<string>, label: string): boolean => open.has(label);
...
export const sidebarWidth = (width: number, collapsed: boolean): number =>
