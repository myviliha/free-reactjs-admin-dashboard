 ...
 *
 * **A plain `<img>` carrying the three attributes `next/image` used to set for it**: `width` and
 * `height` for the layout-shift reservation, `loading` for the lazy fetch, and `sizes` per grid so the
 * browser fetches the width it will actually paint rather than the largest one. A demo of images that
 * skips those is demonstrating the easy half, and they are three attributes rather than a component.
 */
/**
 ...
 * being scaled up more than four times and will look soft. Nothing in the pipeline can invent detail.
 * The fix is larger source files, not a layout change, and until then the grids below are the honest
 * reading of what these can do.
 */
const PHOTOS = [
    ...
    <div className="overflow-hidden rounded-xl border border-border">
      <AspectRatio ratio={ratio}>
        <img
          ...
          // `next/image`'s `priority` was these two attributes. Above the fold the browser is told to
          // fetch eagerly; everything else is lazy, which is what the gallery is demonstrating.
